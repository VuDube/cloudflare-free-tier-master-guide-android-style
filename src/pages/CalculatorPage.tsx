...
import { Calculator, Cpu, Database, Brain, RotateCcw, Activity, Shield, Network, Globe, Video, Image as ImageIcon } from 'lucide-react';
...
export function CalculatorPage() {
  const [, setActiveCategory] = useState('compute');
  const [isStressTesting, setIsStressTesting] = useState(false);
  const [usage, setUsage] = useState<Record<string, number>>(INITIAL_USAGE);
  const calculateUsagePct = useCallback((val: number, limit: number) => 
    Math.min(100, Math.max(0, (val / limit) * 100)), []);
  useEffect(() => {
    let interval: number;
    if (isStressTesting) {
      interval = window.setInterval(() => {
        setUsage(prev => {
          ...
          Object.keys(next).forEach(key => {
            ...
            const delta = (Math.random() - 0.45) * (limit * 0.05);
            next[key] = Math.max(0, Math.min(limit, next[key] + delta));
          });
          return next;
        });
      }, 500);
  ...
  const totalUsage = Object.keys(usage).reduce((acc, key) => 
    acc + calculateUsagePct(usage[key], LIMITS[key as keyof typeof LIMITS]), 0) / Object.keys(usage).length;
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col">
      <div className="py-8 space-y-8 pb-24 overflow-y-auto no-scrollbar flex-1">
        <header className="flex items-center justify-between">
          <div className="space-y-1">
            <h2 className="text-2xl font-sketchy flex items-center gap-2">
              <Calculator className="text-primary" /> Quota 15
            </h2>
          ...
          <div className="h-4 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
            <motion.div 
              ...
              className={cn(
                ...
                totalUsage > 85 ? "bg-destructive" : totalUsage > 50 ? "bg-amber-500" : "bg-emerald-500"
              )} 
            />
          </div>
        </section>
          ...
          {Object.entries(CATEGORIES).map(([catId, items]) => (
            <TabsContent key={catId} value={catId} className="space-y-4">
              {items.map((item) => {
                ...
                const pct = calculateUsagePct(u, l);
                return (
