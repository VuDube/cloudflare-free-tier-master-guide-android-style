...
import { LayoutGrid, Calculator, GraduationCap, Code2, Trash2, RefreshCcw, UserCircle, Activity, Globe, Share2, Terminal, User, Network, Stethoscope, BookOpen } from "lucide-react";
...
export function AppDrawer({ children }: AppDrawerProps) {
  ...
  const navItems = [
    ...
    { label: 'Playbook OS', icon: BookOpen, path: '/playbook' },
    ...
    { label: 'Quota Simulator', icon: Calculator, path: '/calculator' },
    ...
    { label: 'Code & Automation', icon: Code2, path: '/templates' },
  ...
  const handleWipeState = async () => {
    try {
      ...
      localStorage.removeItem('cf_droid_visits');
      toast.success('System state wiped');
      navigate('/');
      window.location.reload();
    } catch (error) { toast.error('Failed to wipe state'); }
  };
  return (
    <Sheet>
      <SheetTrigger asChild>{children}</SheetTrigger>
      <SheetContent side="left" className="w-[300px] p-0 flex flex-col bg-background border-r-2" onOpenAutoFocus={(e) => e.preventDefault()}>
        ...
        <div className="p-4 bg-muted/30 text-center border-t border-dashed">
          <p className="text-[10px] uppercase font-bold tracking-tighter text-muted-foreground">CF DROID OS v2.5.FINAL</p>
        </div>
      </SheetContent>
    </Sheet>
  );
